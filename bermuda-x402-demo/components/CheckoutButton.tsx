...
import { bermudaCheckout, type InspectorStep, type CheckoutResult } from '@/lib/bermuda-client'
...
export function CheckoutButton({ onSuccess }: CheckoutButtonProps) {
  const { itemsParam, clearCart } = useCart()
  const { addStep, clearSteps } = useX402Inspector()
  const { agentPhase, statusMessage: agentStatus, confirmedOrder } = useAgent()
  ...
  // Mirror agent phase into button visual state so the cart button animates
  // while the agent runs bermudaCheckout() directly (without clicking the button).
  useEffect(() => {
    if (agentPhase === 'paying') {
      setState('paying')
      setStatusLabel(agentStatus || 'Paying with x402…')
    } else if (agentPhase === 'confirmed' && confirmedOrder) {
  ...
  }, [agentPhase, agentStatus, confirmedOrder])

  // Keep status label in sync while agent is paying
  useEffect(() => {
  ...
  const handleStep = useCallback((step: InspectorStep) => {
    addStep(step)
    switch (step.type) {
      case 'request_sent':    setStatusLabel('Initiating checkout…'); break
      case '402_received':    setStatusLabel('Payment required — signing…'); break
      case 'account_derived': setStatusLabel('Identity derived ✓'); break
      case 'payload_created': setStatusLabel('Proof created ✓'); break
      case 'payment_sent':    setStatusLabel('Submitting payment…'); break
      case 'success':         setStatusLabel('Confirmed ✓'); break
      case 'error':           setStatusLabel(step.message); break
    }
  }, [addStep])
  ...
  }, [itemsParam, clearSteps, handleStep, clearCart, onSuccess])
    ...
    return (
      <div className="w-full py-3.5 rounded-xl bg-bermuda-900/40 border border-bermuda-800/30 text-center px-4">
        <div className="flex items-center justify-center gap-2 text-bermuda-500 text-sm">
          ...
          <span>Add <code className="text-xs bg-bermuda-900 px-1 rounded">NEXT_PUBLIC_AGENT_PK</code> to enable payments</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <>
      ...
      {state === 'success' && order && (
        <ConfirmationModal
          order={order}
          onClose={() => { setState('idle'); setOrder(null) }}
        />
      )}
    </>
  )
